...
/* iOS/macOS uses dispatch_semaphore instead of POSIX semaphores */
...
sem_trywait_block(sem_t *sem, unsigned long timeout)
{
	...
	if (timeout == 0) {
		wait_time = DISPATCH_TIME_NOW;
	} else {
		wait_time = dispatch_time(DISPATCH_TIME_NOW, (int64_t)timeout * NSEC_PER_MSEC);
	}
	long result = dispatch_semaphore_wait(*sem, wait_time);
	if (result != 0) {
		errno = EAGAIN;
		return -1;
	}
