...
extern "C" {
		...
		/* iOS/macOS doesn't support unnamed POSIX semaphores - use dispatch_semaphore */
		#include <dispatch/dispatch.h>
		typedef dispatch_semaphore_t sem_t;
		#define     sem_init(psem, pshared, value) (*(psem) = dispatch_semaphore_create(value), (*(psem) == NULL) ? -1 : 0)
		#define     sem_destroy(psem)   (dispatch_release(*(psem)), 0)
		#define     sem_wait(psem)      dispatch_semaphore_wait(*(psem), DISPATCH_TIME_FOREVER)
		#define     sem_trywait(psem)   (dispatch_semaphore_wait(*(psem), DISPATCH_TIME_NOW) == 0 ? 0 : (errno = EAGAIN, -1))
