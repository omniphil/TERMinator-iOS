...
static char *chr(int ch)
{
	...
	if (ch < 0)
		snprintf(str, sizeof(str), "%d", ch);
	else if (ch >= ' ' && ch <= '~')
		snprintf(str, sizeof(str), "'%c' (%02Xh)", (uchar)ch, (uchar)ch);
	else
		snprintf(str, sizeof(str), "%u (%02Xh)", (uchar)ch, (uchar)ch);
	return str;
}
...
static char* frame_desc(int frame)
{
	...
	if (frame >= 0 && (frame & BADSUBPKT)) {
		switch (frame & ~BADSUBPKT) {
			case ZRQINIT:       name = "ZRQINIT";      break;
			case ZRINIT:        name = "ZRINIT";       break;
			case ZSINIT:        name = "ZSINIT";       break;
			case ZACK:          name = "ZACK";         break;
			case ZFILE:         name = "ZFILE";        break;
			case ZSKIP:         name = "ZSKIP";        break;
			case ZNAK:          name = "ZNAK";         break;
			case ZABORT:        name = "ZABORT";       break;
			case ZFIN:          name = "ZFIN";         break;
			case ZRPOS:         name = "ZRPOS";        break;
			case ZDATA:         name = "ZDATA";        break;
			case ZEOF:          name = "ZEOF";         break;
			case ZFERR:         name = "ZFERR";        break;
			case ZCRC:          name = "ZCRC";         break;
			case ZCHALLENGE:    name = "ZCHALLENGE";   break;
			case ZCOMPL:        name = "ZCOMPL";       break;
			case ZCAN:          name = "ZCAN";         break;
			case ZFREECNT:      name = "ZFREECNT";     break;
			case ZCOMMAND:      name = "ZCOMMAND";     break;
			case ZSTDERR:       name = "ZSTDERR";      break;
			default:
				snprintf(str, sizeof(str), "Unknown (%08X)", frame);
				return str;
		}
		snprintf(str, sizeof(str), "BAD %s", name);
	} else
		return chr(frame);
	return str;
}
...
//	lprintf(zm,LOG_DEBUG, __FUNCTION___);
	...
	*type = subpkt_type;
	...
	if (rxd_crc != crc) {
		/* CR+NUL recovery: telnet servers in text mode strip NUL (0x00) after
		 * CR (0x0D), losing 1 byte per CR+NUL pair. Try reinserting NUL after
		 * subsets of CR positions and recheck CRC32 to recover original data.
		 * Typically ~4 CRs per 1024 bytes, so 2^4=16 attempts max.
		 * Brute-force limited to 20 CRs (2^20 = ~1M combinations). */
		int recovered = FALSE;
		unsigned cr_pos[128];
		int cr_count = 0;
		unsigned i;

		for (i = 0; i < *len && cr_count < 128; i++) {
			if (start_p[i] == 0x0D)
				cr_pos[cr_count++] = i;
		}

		if (cr_count > 0 && cr_count <= 20) {
			uint32_t mask;
			uint32_t max_mask = (1u << cr_count) - 1;
			for (mask = 1; mask <= max_mask && !recovered; mask++) {
				int nbits = 0;
				uint32_t tmp = mask;
				while (tmp) { nbits += tmp & 1; tmp >>= 1; }
				...
				/* Compute CRC with NUL inserted after selected CRs */
				uint32_t try_crc = 0xffffffffl;
				unsigned j;
				int ci = 0;
				for (j = 0; j < *len; j++) {
					try_crc = ucrc32(start_p[j], try_crc);
					if (ci < cr_count && j == cr_pos[ci]) {
						if (mask & (1u << ci))
							try_crc = ucrc32(0x00, try_crc);
						ci++;
					}
				}
				try_crc = ucrc32(subpkt_type, try_crc);
				try_crc = ~try_crc;

				if (try_crc == rxd_crc) {
					/* CRC matches - insert the NUL bytes (work backwards) */
					int k;
					for (k = cr_count - 1; k >= 0; k--) {
						if (!(mask & (1u << k)))
							continue;
						unsigned pos = cr_pos[k] + 1;
						if (pos >= *len || *len + 1 > maxlen)
							break;
						memmove(start_p + pos + 1, start_p + pos, *len - pos);
						start_p[pos] = 0x00;
						(*len)++;
					}
					lprintf(zm, LOG_WARNING, "%lu CR+NUL recovery: inserted %d NUL byte(s)",
						(ulong)zm->ack_file_pos, nbits);
					recovered = TRUE;
				}
			}
		}
...
BOOL zmodem_send_file(zmodem_t* zm, char* fname, FILE* fp, BOOL request_init, time_t* start, uint64_t* sent)
{
	...
	{
		size_t remaining = sizeof(zm->tx_data_subpacket) - (size_t)(p - zm->tx_data_subpacket);
		snprintf((char*)p, remaining, "%" PRId64 " %" PRIoMAX " 0 0 %u %" PRId64 " 0"
...
void zmodem_parse_zfile_subpacket(zmodem_t* zm)
{
	...
	/* Ensure NUL terminator exists within buffer before strlen/sscanf */
	zm->rx_data_subpacket[sizeof(zm->rx_data_subpacket) - 1] = '\0';
	...
	{
		size_t name_len = strlen((char*)zm->rx_data_subpacket);
		if (name_len + 1 >= sizeof(zm->rx_data_subpacket)) {
			/* No room for metadata after filename - skip sscanf */
			i = 0;
		} else {
			i = sscanf((char*)zm->rx_data_subpacket + name_len + 1, "%" SCNd64 " %lo %o %lo %u %" SCNd64
