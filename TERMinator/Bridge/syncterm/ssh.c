...
init_crypt(void)
{
	...
	printf("[SSH] Initializing cryptlib...\n");
	status = cryptInit();
	...
	if (cryptStatusError(status)) {
		printf("[SSH] cryptlib init FAILED (status=%d)\n", status);
		cryptlib_error_message(status, "initializing cryptlib");
	} else {
		printf("[SSH] cryptlib initialized OK\n");
	}
}
...
{
	printf("[SSH] error_popup: %s (status=%d)\n", blurb, status);
...
ssh_connect(struct bbslist *bbs)
{
	...
	if (cryptStatusError(status)) {
		printf("[SSH] Key management failed (status=%d), will try password auth\n", status);
		cryptDestroyContext(ssh_context);
		ssh_context = -1;
	}

	// Key management errors are recoverable (password auth still works).
	// Clear terminate flag that error_popup may have set.
	conn_api.terminate = false;

	printf("[SSH] Connecting socket to %s:%d...\n", bbs->addr, bbs->port);
	ssh_sock = conn_socket_connect(bbs, true);
	if (ssh_sock == INVALID_SOCKET) {
		printf("[SSH] Socket connection failed\n");
		return -1;
	}
	printf("[SSH] Socket connected (fd=%d)\n", ssh_sock);
	...
	printf("[SSH] Auth: username='%s', password=%s, has_key=%d\n",
	       username, password[0] ? "(set)" : "(empty)", ssh_context != -1);

	if (!bbs->hidepopups)
		uifc.pop(NULL);

	if (!username[0]) {
		printf("[SSH] WARNING: No username provided\n");
		if (bbs->hidepopups)
	...
	printf("[SSH] Activating session (SSH handshake)...\n");
	if (!bbs->hidepopups) {
		uifc.pop(NULL);
	...
	do {
		...
		if (status == CRYPT_ENVELOPE_RESOURCE) {
			printf("[SSH] Server requested password (CRYPT_ENVELOPE_RESOURCE)\n");
			if (bbs->hidepopups) {
				// On iOS we can't prompt interactively - fail with auth error
				printf("[SSH] Cannot prompt for password (hidepopups=1), failing\n");
				break;
			}
	...
	if (cryptStatusError(status)) {
		printf("[SSH] Session activation FAILED (status=%d)\n", status);
		free(pubkey);
		error_popup(bbs, "activating session", status);
		ssh_close();
		return -1;
	}
	printf("[SSH] Session activated successfully\n");
	FlushData(ssh_session);
	...
	printf("[SSH] ssh_connect completed successfully\n");
	return 0;
}
