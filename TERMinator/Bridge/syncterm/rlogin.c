...
// RLogin logging - disabled for now
#define RLOGIN_LOG(fmt, ...) ((void)0)
...
rlogin_input_thread(void *args)
{
	...
	RLOGIN_LOG("[rlogin_input_thread] Starting");
	SetThreadName("RLogin Input");
	conn_api.input_thread_running = 1;
	RLOGIN_LOG("[rlogin_input_thread] Set running=1 sock=%d terminate=%d", rlogin_sock, conn_api.terminate);
	while (rlogin_sock != INVALID_SOCKET && !conn_api.terminate) {
		...
		if (socket_check(rlogin_sock, &data_avail, NULL, bufsz ? 0 : 100)) {
			if (data_avail && bufsz < BUFFER_SIZE) {
				...
				if (rd <= 0) {
					RLOGIN_LOG("[rlogin_input_thread] recv returned %d, breaking", rd);
					break;
				}
				bufsz += rd;
			}
			...
			RLOGIN_LOG("[rlogin_input_thread] socket_check returned false, breaking");
			break;
		}
	}
	RLOGIN_LOG("[rlogin_input_thread] Exiting loop sock=%d terminate=%d", rlogin_sock, conn_api.terminate);
	conn_api.terminate = true;
	shutdown(rlogin_sock, SHUT_RDWR);
	conn_api.input_thread_running = 2;
	RLOGIN_LOG("[rlogin_input_thread] Thread ending, running=2");
}
...
rlogin_output_thread(void *args)
{
	...
	RLOGIN_LOG("[rlogin_output_thread] Starting");
	SetThreadName("RLogin Output");
	conn_api.output_thread_running = 1;
	RLOGIN_LOG("[rlogin_output_thread] Set running=1");
	while (rlogin_sock != INVALID_SOCKET && !conn_api.terminate) {
		...
		if (ret < 0) {
			RLOGIN_LOG("[rlogin_output_thread] ret < 0, breaking");
			break;
		}
	}
	RLOGIN_LOG("[rlogin_output_thread] Exiting loop");
	conn_api.terminate = true;
	shutdown(rlogin_sock, SHUT_RDWR);
	conn_api.output_thread_running = 2;
	RLOGIN_LOG("[rlogin_output_thread] Thread ending, running=2");
}
