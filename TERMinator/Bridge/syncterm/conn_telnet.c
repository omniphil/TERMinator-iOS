...
{
	printf("[telnet_connect] Starting\n");
	if (!bbs->hidepopups)
		init_uifc(true, true);
	...
	printf("[telnet_connect] conn_socket_connect returned socket=%d\n", rlogin_sock);
	if (rlogin_sock == INVALID_SOCKET)
		return -1;

	printf("[telnet_connect] Creating input buffer...\n");
	if (!create_conn_buf(&conn_inbuf, BUFFER_SIZE)) {
		printf("[telnet_connect] Failed to create input buffer!\n");
		closesocket(rlogin_sock);
		rlogin_sock = INVALID_SOCKET;
		return -1;
	}
	printf("[telnet_connect] Creating output buffer...\n");
	if (!create_conn_buf(&conn_outbuf, BUFFER_SIZE)) {
		printf("[telnet_connect] Failed to create output buffer!\n");
		destroy_conn_buf(&conn_inbuf);
		closesocket(rlogin_sock);
		rlogin_sock = INVALID_SOCKET;
		return -1;
	}
	printf("[telnet_connect] Allocating read buffer...\n");
	conn_api.rd_buf = (unsigned char *)malloc(BUFFER_SIZE);
	if (!conn_api.rd_buf) {
		printf("[telnet_connect] Failed to allocate read buffer!\n");
		destroy_conn_buf(&conn_inbuf);
	...
	printf("[telnet_connect] Allocating write buffer...\n");
	conn_api.wr_buf = (unsigned char *)malloc(BUFFER_SIZE);
	if (!conn_api.wr_buf) {
		printf("[telnet_connect] Failed to allocate write buffer!\n");
	...
	printf("[telnet_connect] Buffers all created successfully\n");
	...
	printf("[telnet_connect] Returning 0 (success)\n");
	return 0;
}
